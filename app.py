...
from models import irradiance_model as _irradiance_model

# O Streamlit Community Cloud pode manter módulos Python em memória durante um
# hot reload. Se app.py for atualizado antes de models/irradiance_model.py,
# o processo pode conservar a versão antiga do módulo e gerar um ImportError
# mesmo depois que o arquivo correto chega ao GitHub. A verificação abaixo
# recarrega o módulo local quando a API esperada não está disponível.
_IRRADIANCE_API_VERSION = "2026.07.22.2"
_IRRADIANCE_REQUIRED_NAMES = (
    "IRRADIANCE_PROFILES",
    "SEASONS",
...
if (
    getattr(_irradiance_model, "PROFILE_LOADER_API_VERSION", None)
    != _IRRADIANCE_API_VERSION
    or not all(hasattr(_irradiance_model, name) for name in _IRRADIANCE_REQUIRED_NAMES)
):
    importlib.invalidate_caches()
    _irradiance_model = importlib.reload(_irradiance_model)

_missing_irradiance_api = [
    name for name in _IRRADIANCE_REQUIRED_NAMES
    if not hasattr(_irradiance_model, name)
]
if _missing_irradiance_api:
    raise ImportError(
        "Arquivos incompatíveis no deploy. Atualize app.py e "
        "models/irradiance_model.py no mesmo commit. Elementos ausentes: "
        + ", ".join(_missing_irradiance_api)
    )

IRRADIANCE_PROFILES = _irradiance_model.IRRADIANCE_PROFILES
SEASONS = _irradiance_model.SEASONS
build_synthetic_profile = _irradiance_model.build_synthetic_profile
detect_profile_columns = _irradiance_model.detect_profile_columns
infer_timestep_hours = _irradiance_model.infer_timestep_hours
prepare_custom_profile = _irradiance_model.prepare_custom_profile
read_custom_profile_table = _irradiance_model.read_custom_profile_table
